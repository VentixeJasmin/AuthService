...
public class AuthController(UserManager<UserEntity> userManager, UserService userService, IConfiguration configuration) : ControllerBase
{
    ...
    public async Task<IActionResult> ConfirmEmail([FromBody] ConfirmEmailRequest request)
    {
        ...
        //whole try/catch statement generated by Claude AI for safer better api calls 
        try
        {
            ...
            var profileResult = await httpClient.PostAsJsonAsync("https://profileservice-jdf-gbeac0bjb8c0gegm.swedencentral-01.azurewebsites.net/api/profiles", profileDto);

            if (!profileResult.IsSuccessStatusCode)
            {
                var errorContent = await profileResult.Content.ReadAsStringAsync();
            }
        }
        catch (HttpRequestException ex)
        {
            Console.WriteLine("Failed to communicate with profile service", ex);
        }
